package {
	...
	public class FleeTarget {
		...
		private var overhead:Sprite;
		public function FleeTarget(parent:DisplayObjectContainer, bg:DisplayObjectContainer, pX:int, pY:int) {
			goreBits = [];
			health = 50;
			dead = false;
			if (Math.random() > 0.5) {
			...
			icon.rotation = ang;
			overhead = new guyTarrget();
			overhead.alpha = 0;
			overhead.x = pX;
			overhead.y = pY;
			_parent.addChild(icon);
			_parent.addChild(overhead);
			prevrot = 0;
		}
		public function cleanUp():void {
			...
			icon = null;
			_parent.removeChild(overhead);
			overhead = null;
		}
		public function update(p:Sprite, b:Array, dX:Number, dY:Number, pos:int, w:int, h:int, mX:int, mY:int):uint {
			...
			if (!dead) {
				...
				if (shot > 0) {
					shootSomeBloodEverywhere((10 / (health - shot + 1)) * shot, vec.angle);
					health -= shot;
					if (health <= 0) {
						var px:int = icon.x;
						var py:int = icon.y;
						...
						PeopleParser.Wordle.dialogue.text = "Assassination Complete.";
						PeopleParser.pushTarget();
				...
				icon.x += 3 * Math.cos((ang + 90) * Math.PI / 180);
				icon.y += 3 * Math.sin((ang + 90) * Math.PI / 180);
				if (icon.x > (mX + w)) {
					icon.x = mX + w;
				} else if (icon.x < (mX - w)) {
					icon.x = mX - w;
		...
		private function shootSomeBloodEverywhere(goreMagnitude:int, angle:int):void {
			var gore:int = 20 * (goreMagnitude / 2 + goreMagnitude * Math.random() + 5 * Math.random());
			if (goreMagnitude > 15) {
				gore *= 2;
			}
			if (gore < 10) {
