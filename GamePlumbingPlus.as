package {
	...
	public class GamePlumbingPlus {
		...
		public var justOneRound:Boolean;
		public function GamePlumbingPlus(playerL:DisplayObjectContainer, bg:DisplayObjectContainer, hud:DisplayObjectContainer, bbg:DisplayObjectContainer, mapSpeed:Number = 1.5, spawnrate:Number = 1, enemy:int = 1, HP:int = 1, justOne:Boolean = false) {
			justOneRound = justOne;
			_pLayer = playerL;
			_xLayer = bbg;
			_bLayer = bg;
			_hLayer = hud;
			...
			var hhhh:int = Math.ceil(health / 3);
		...
		private function iWillStopYouEvilBoss(bosss:int):void {
			switch(bosss) {
				case 6:
					...
					break;
				case 5:
					boss = new Deceptus(_bLayer, 1820, 125, true);
					break;
				case 4:
					boss = new Monogamy(_bLayer, 1872, 120, true);
					break;
				case 3:
					boss = new Chairs(_bLayer, 1820, 120, true);
					break;
				case 2:
					boss = new TheQuery(_bLayer, 1820, 125, true);
					break;
				case 1:
				default:
					boss = new BabyDearest(_bLayer, 1820, 125, true);
					break;
			}
		}
		...
		private function addElementsToPipe(spawnrate:Number):void {
			pipeElements = [];
			var counter:int = 0;
			while ((Math.random() < 0.85 || counter < 100) && counter < 500) {
				counter += 1;
			...
			counter = 0;
			while ((Math.random() < 0.65 || counter < 25) && counter < 100) {
				counter += 1;
			...
			while (Math.random() < 0.95 && counter < 150) {
				...
				dbris.gotoAndStop(6 + Math.ceil(Math.random() * 10));
